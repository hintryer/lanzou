...
echo.
mkdir "下载传输" 2>nul
echo 📥 下载 → 下载传输 \ lanzouyun-3.5.3-x64-win.zip
curl -L -# -o "下载传输\lanzouyun-3.5.3-x64-win.zip" "https://gh-proxy.com/https://github.com/chenhb23/lanzouyun-disk/releases/download/v3.5.3/lanzouyun-3.5.3-x64-win.zip"
echo.
mkdir "其它工具" 2>nul
echo 📥 下载 → 其它工具 \ Clash.Verge_2.5.1_x64-setup.exe
curl -L -# -o "其它工具\Clash.Verge_2.5.1_x64-setup.exe" "https://gh-proxy.com/https://github.com/clash-verge-rev/clash-verge-rev/releases/download/v2.5.1/Clash.Verge_2.5.1_x64-setup.exe"
echo.
mkdir "浏览器相关" 2>nul
...
curl -L -# -o "浏览器相关\chrome_150.0.7871.47_uncompressed.exe" "https://gh-proxy.com/https://github.com/Bush2021/chrome_installer/releases/download/150.0.7871.47/x64_150.0.7871.47_chrome_installer_uncompressed.exe"
echo.
mkdir "影音工具" 2>nul
...
echo.
mkdir "文本办公" 2>nul
echo 📥 下载 → 文本办公 \ typora-plugin@v1.18.6.zip
curl -L -# -o "文本办公\typora-plugin@v1.18.6.zip" "https://gh-proxy.com/https://github.com/obgnail/typora_plugin/releases/download/1.18.6/typora-plugin%40v1.18.6.zip"
echo.
mkdir "图像相关" 2>nul
...
echo.
mkdir "运行库" 2>nul
echo 📥 下载 → 运行库 \ dotnet-runtime-7.0.20-win-x64.exe
...
echo.
mkdir "运行库" 2>nul
echo 📥 下载 → 运行库 \ dotnet-runtime-10.0.9-win-x64.exe
curl -L -# -o "运行库\dotnet-runtime-10.0.9-win-x64.exe" "https://builds.dotnet.microsoft.com/dotnet/Runtime/10.0.9/dotnet-runtime-10.0.9-win-x64.exe"
echo.
mkdir "运行库" 2>nul
...
curl -L -# -o "影音工具\PotPlayer-260422.exe" "https://dlink.host/1drv/aHR0cHM6Ly8xZHJ2Lm1zL3UvYy9kMTk2Y2Y1YmZjNWM1YmY3L0lRQlhNb0lHNk9HLVJyb0lLS3cxMHlaSUFVOV9CR2FvVlVRanF4TlBISEMtWXJrP2U9c01TelJk.exe"
echo.
mkdir "其它工具" 2>nul
...
echo.
mkdir "运行库" 2>nul
echo 📥 下载 → 运行库 \ dotnet-runtime-9.0.17-win-x64.exe
curl -L -# -o "运行库\dotnet-runtime-9.0.17-win-x64.exe" "https://builds.dotnet.microsoft.com/dotnet/Runtime/9.0.17/dotnet-runtime-9.0.17-win-x64.exe"
echo.
mkdir "文本办公" 2>nul
echo 📥 下载 → 文本办公 \ EmEditor_portable.zip
curl -L -# -o "文本办公\EmEditor_portable.zip" "https://download.emeditor.info/emed64_24.1.0_portable.zip"
echo.
mkdir "运行库" 2>nul
echo 📥 下载 → 运行库 \ dotnet-runtime-8.0.28-win-x64.exe
curl -L -# -o "运行库\dotnet-runtime-8.0.28-win-x64.exe" "https://builds.dotnet.microsoft.com/dotnet/Runtime/8.0.28/dotnet-runtime-8.0.28-win-x64.exe"
echo.
mkdir "运行库" 2>nul
...
echo.
mkdir "浏览器相关" 2>nul
echo 📥 下载 → 浏览器相关 \ SingleFile_1.22.98.crx
...
echo.
mkdir "浏览器相关" 2>nul
echo 📥 下载 → 浏览器相关 \ Immersive_Translate_1.31.2.crx
curl -L -# -o "浏览器相关\Immersive_Translate_1.31.2.crx" "https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx2,crx3&prodversion=130.0.0.0&x=id%3Dbpoadfkcbjbfhfodiogcnhhhpibjhbnh%26uc"
echo.
echo ==============================================
echo                 ✅ 全部下载完成
echo ==============================================
